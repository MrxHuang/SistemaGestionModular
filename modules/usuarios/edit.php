...
// Get user data
$query = "SELECT id, nombre, apellido, email, rol FROM usuarios WHERE id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["email"]))) {
        ...
        // Check if email exists (only if email has changed)
    ...
    if (empty($nombre_err) && empty($apellido_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($rol_err)) {
        ...
        if (!empty($password)) {
            ...
            $sql = "UPDATE usuarios SET nombre = :nombre, apellido = :apellido, email = :email, password = :password, rol = :rol WHERE id = :id";
            $stmt = $db->prepare($sql);
...
<div class="container px-6 py-8 mx-auto">
    ...
    <div class="bg-white shadow-md rounded-lg p-6">
        <form action="?module=usuarios&page=edit&id=<?php echo $id; ?>" method="post">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-1">Nueva Contraseña <span class="text-gray-500 text-xs">(Dejar en blanco para mantener la actual)</span></label>
